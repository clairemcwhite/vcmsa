...
dat_formatted_new <- rbind(dat_formatted_fwd, dat_formatted_rev)

dat_formatted_new %>% full_join(dat_formatted, by = c("protein", "fasta", "seq1", "seq2", "aa1", "aa2")) %>%
  filter(protein == "6PF2K")
...
allscores_standard <- allscores %>%
  ...
  filter(score_type %in% c("tc", "tcHGI", "tcHGIEB")) %>%
  separate(experiment, into = c("tmp0", "tmp1", 'set', "tmp2", 'layers', 'model', 'padding', 'threshold', 'bc', 'rbh', 'tmp3'), sep = "_", extra = "merge", remove = FALSE) %>%
...
get_layer_compare <- function(allscores_standard, experiment_name, score_type = "tc", ref = "20seqs_prog") {

layer_compare_sel <- allscores_standard %>%
    filter(experiment %in% c({{experiment_name}}, {{ref}})) %>% # 
  left_join(sims) %>%
  unique %>%
...
a <- perform_plot_fxn(score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust , order2)
a
...
bind_rows(score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust, add_extra)
...
```{r}
score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust <- allscores_standard %>%
  get_layer_compare(., "aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust", score_type = "tc") %>%
  rename(vcMSA = semantic)

score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_zcbc_mnc_oldclust <- allscores_standard %>%
  get_layer_compare(., "aln_evaluate_20_nopca_16layer_t5_padding10_thr70_zcbc_mnc_oldclust", score_type = "tc") %>%
  rename(vcMSA = semantic)

score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_none_oldclust <- allscores_standard %>%
  get_layer_compare(., "aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_none_oldclust", score_type = "tc") %>%
  rename(vcMSA = semantic)


#scoring
score_compare_aln_evaluate_20_nopca_16layer_t5_padding0_thr70_bc_mnc_oldclust <- allscores_standard %>%
  get_layer_compare(., "aln_evaluate_20_nopca_16layer_t5_padding0_thr70_bc_mnc_oldclust", score_type = "tc") %>%
  rename(vcMSA = semantic)

score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust <- allscores_standard %>%
  get_layer_compare(., "aln_evaluate_20_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust", score_type = "tc") %>%
  rename(vcMSA = semantic)



score_compare_aln_evaluate_50_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust <- allscores_standard %>%
  get_layer_compare(., "aln_evaluate_50_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust", score_type = "tc", ref = "50seqs_prog") %>%
  rename(vcMSA = semantic)
...
add_extra <- score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust %>% filter(alg == "CLUSTALO") %>%
    mutate(alg = "extra")
...
plot_16_mainref <- score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust %>% filter(alg != "MAFFT-LINSI") %>%
                                                                   bind_rows(add_extra) %>%
...
plot_grid(plot_16_main, plot_16_mainref)
...
add_extra <- score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust %>% filter(alg == "CLUSTALO") %>%
    mutate(alg = "extra")
...
plot_16_main <- score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust %>% filter(alg != "MAFFT-LINSI") %>%
                                                                   bind_rows(add_extra) %>%
  perform_plot_fxn(order3)
...
plot_16_main_ref <- score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust %>% filter(alg != "MAFFT-LINSI") %>%
                                                                   bind_rows(add_extra) %>%
  perform_plot_fxn(order3)


plot_grid(plot_16_main, plot_16_main_ref , plot_16_main_50, nrow = 1)
plot_16_main %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/plot_16_main.png", ., height = 5, width = 5.8,, units = "in", dpi = 600)
plot_16_main%>%  ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/plot_16_main.pdf", ., height = 5, width = 5.8, units = "in")
...
score_compare_aln_evaluate_50_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust <- allscores_standard %>%
  get_layer_compare(., "aln_evaluate_50_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust", score_type = "tc") %>%
  rename(vcMSA = semantic)


add_extra <- score_compare_aln_evaluate_50_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust %>% filter(alg == "CLUSTALO") %>%
    mutate(alg = "extra")


order3 <-c("extra vs. vcMSA", "MAFFT-FFTNS1 vs. vcMSA" ,'PROBCONS vs. vcMSA', "MUSCLE vs. vcMSA",  'FAMSA vs. vcMSA', "UPP vs. vcMSA", "CLUSTALO vs. vcMSA", "MAFFT-GINSI vs. vcMSA", "TCOFFEE vs. vcMSA")

plot_16_main_50 <- score_compare_aln_evaluate_50_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust %>% filter(alg != "MAFFT-LINSI") %>%
                                                                   bind_rows(add_extra) %>%
  perform_plot_fxn(order3)


plot_16_main_50
plot_16_main_50 %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/plot_16_main_50.png", ., height = 5, width = 5.8,, units = "in", dpi = 600)
plot_16_main_50%>%  ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/plot_16_main_50.pdf", ., height = 5, width = 5.8, units = "in")
...
Win percent
```{r}
score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust%>%
           #filter(!prot %in% extreme_length_diffs) %>%
...
score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust%>%
           filter(!prot %in% extreme_length_diffs) %>%
  mutate(win = case_when(vcMSA >= other ~ "Win",
                         TRUE ~ "Lose")) %>%
  group_by(score_type, alg, win) %>%
...
score_compare_aln_evaluate_50_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust%>%
           #filter(!prot %in% extreme_length_diffs) %>%
  mutate(win = case_when(vcMSA >= other ~ "Win",
                         TRUE ~ "Lose")) %>%
  group_by(score_type, alg, win) %>%
...
score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust
score_compare_aln_evaluate_20_nopca_16layer_t5_padding0_thr70_bc_mnc_oldclust

pad_compare <- score_compare_aln_evaluate_20_nopca_16layer_t5_padding0_thr70_bc_mnc_oldclust %>% 
    filter(!prot %in% extreme_length_diffs) %>%
  left_join(score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust, by= c("prot" ,"score_type", "other", "identbin", "same", "sim", "alg")) %>%
...
padding_plot %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/padding_plot.png", ., height = 1.4, width = 1.4, units = "in", dpi = 600)
padding_plot%>%  ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/padding_plot.pdf", ., height = 1.4, width = 1.4, units = "in")
...
score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_nobc_mnc_oldclust <- allscores_standard %>%

     #filter(!prot %in% extreme_length_diffs) %>%
  get_layer_compare(., "compare_aln_evaluate_20_nopca_16layer_t5_padding10_thr70_nobc_mnc_oldclust", score_type = "tc") %>%
  rename(vcMSA = semantic)


nobc <- allscores_standard %>% filter(experiment %in% c("aln_evaluate_20_nopca_16layer_t5_padding10_thr70_nobc_mnc_oldclust")) %>% select(prot, score_type,score)
bc <- allscores_standard %>% filter(experiment %in% c("aln_evaluate_20_nopca_16layer_t5_padding10_thr70_refbc_mnc_oldclust")) %>% select(prot, score_type, score) 

batch_compare <- nobc %>% 
  left_join(bc, by= c("prot" ,"score_type")) %>%
    filter(!prot %in% extreme_length_diffs) %>%
  mutate(color = case_when(score.x >= score.y ~ "win", TRUE ~ "lose")) %>%
  filter(score_type == "tc") %>%
  unique



#batch_compare <- nobc %>% full_join(bc, by = c("prot", "score_type")) %>% filter(score_type == "tc")  #%>%
  #ggplot(aes(x = score.x, y = score.y)) +
  #geom_point() +
  #geom_abline(yintercept = 1, slope = 1)




batching_plot <- batch_compare %>%  
  filter(!prot %in% extreme_length_diffs) %>%
  ggplot(aes(x = score.x, y = score.y, label = prot, color = color)) +
  geom_abline() + 
  geom_point(size= 0.8, alpha = 0.8)  +
  xlab("No batch correction\nTotal column score") + 
...
batching_plot %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/batching_plot.png", ., height = 2, width = 2, units = "in", dpi = 600)
batching_plot%>%  ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/batching_plot.pdf", ., height = 2, width = 2, units = "in")
...
```{r}
pad_batch <- plot_grid(padding_plot, batching_plot, ncol = 1, align = "hv")

pad_batch %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/pad_batch.png", ., height = 3, width = 1.6, units = "in", dpi = 600)
pad_batch%>%  ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/pad_batch.pdf", ., height = 3, width = 1.6, units = "in")
...
semantic <- allscores_standard %>%
        # filter(!prot %in% extreme_length_diffs) %>%
  filter(experiment == "aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust") %>%
  filter(score_type == "tc") %>%
  mutate(alg = "vcMSA")
...
allseq_scores <- allscores_standard %>%
  filter(!prot %in% extreme_length_diffs) %>%
    filter(score_type == "tc") %>% 
    filter(experiment == "aln_evaluate_20_nopca_16layer_t5_padding10_thr70_bc_mnc_oldclust" | experiment ==  "20seqs_prog") %>%
 left_join(sims2) %>%
...
```{r}
                                     #alignment_files_ghf34_3.fasta.thresh0.70.13.t5.bc/ghf34_3.fasta.thresh0.70.13.t5.bc.alignment_group2.prebatch.png.pca.csv
...
# This is using the reference batch correction
prebatch <- read_csv("/home/cmcwhite/alignment_files_ghf34_3.fasta.vcmsa.thsh0.70.padding10.16.t5half.pycomabtbc_ref.mnc.4.aln/ghf34_3.fasta.vcmsa.thsh0.70.padding10.16.t5half.pycomabtbc_ref.mnc.4.aln.alignment_group0.prebatch.png.pca.csv") %>% mutate(set ="Before\nbatch correction")
postbatch <- read_csv("/home/cmcwhite/alignment_files_ghf34_3.fasta.vcmsa.thsh0.70.padding10.16.t5half.pycomabtbc_ref.mnc.4.aln/ghf34_3.fasta.vcmsa.thsh0.70.padding10.16.t5half.pycomabtbc_ref.mnc.4.aln.alignment_group0.postbatch.png.pca.csv") %>% mutate(set ="After\nbatch correction")
...
bc_plot <- prepostbatch %>%
  ...
  facet_wrap(~fct_rev(set), nrow = 1, scales = "free") +
...
metrics <- read_csv("/scratch/gpfs/cmcwhite/aln_comparison//eval_metrics.csv", col_names = FALSE) %>%
  ...
  filter(!is.na(prot)) %>%
  separate(experiment, into = c('set', NA, 'numseqs', NA, 'model'), sep = "_", extra = "merge", remove = FALSE) %>% unique


metrics_prog <- metrics %>% filter(grepl("prog", experiment)) %>%
  # filter(alg == "CLUSTALO") %>%
  pivot_wider(names_from = "eval", values_from = "score") %>%
  filter(set == "100seqs") #%>%
  ggplot(aes(x = set, y = (realtime/1000)/60, group = prot)) +
  geom_line(alpha = 0.1) + 
  geom_point(alpha = 0.1) +
  facet_wrap(~alg)

metrics_sel_nobc <- metrics %>% # filter(prot %in% c("AhpC-TSA", "bromodomain",  "lacI", "Mur_ligase_C", "sh2")) %>%
  filter(alg == "semantic") %>%
  filter(grepl("padding10_thr90_nobc_mnc_oldclust", experiment)) %>%
    filter(!grepl("prev", experiment)) %>%
  pivot_wider(names_from = "eval", values_from = "score")

metrics_sel_refbc <- metrics %>% # filter(prot %in% c("AhpC-TSA", "bromodomain",  "lacI", "Mur_ligase_C", "sh2")) %>%
  filter(alg == "semantic") %>%
  filter(grepl("padding10_thr70_refbc_mnc_oldclust", experiment)) %>%
  filter(!grepl("prev", experiment)) %>%
  pivot_wider(names_from = "eval", values_from = "score")



metrics_200_ref <- metrics_sel_refbc %>% filter(grepl("200", experiment)) %>% sample_n(100)
metrics_200_nobc <- metrics_sel_nobc %>% filter(grepl("200", experiment))# %>% sample_n(50)

metrics_200_both <- metrics_200_ref %>% inner_join(metrics_200_nobc, by = c("prot"))

metrics_200_both
bind_rows(metrics_200_nobc, metrics_200_ref) %>%
 group_by(prot) %>%
     summarize(n = n()) %>%
  filter(n==2)
...
# In text values
metrics_sel_refbc %>% group_by(experiment) %>%
  
    filter(prot %in% metrics_200_ref$prot) %>%
    summarize(t = median((realtime/1000)/60), min = min((realtime/1000)/60), max = max((realtime/1000)/60))
...
library(cowplot)
theme_set(theme_cowplot(font_size = 8))
timing_plot1 <- metrics_sel_refbc %>% #select(numseqs, eval, score, prot) %>%
 filter(prot %in% metrics_200_ref$prot) %>%
...
timing_plot1 %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/timing_plot.png", ., height = 2.5, width = 3, units = "in", dpi = 600)
timing_plot1 %>%  ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/timing_plot.pdf", ., height = 2.5, width = 3, units = "in")
...
rbh_sims <- dat_formatted_none %>% select(protein, seq1, seq2, aa1, aa2, sim)
...
mncm_lab1 <- subset(dat_formatted_ref, label==1) # %>% select(-sim) %>% left_join(rbh_sims)
mncm_lab1$set = "mncm_1"
none_labNA <- subset(dat_formatted_ref_none, is.na(label))
none_labNA$set = "none_NA#"
mncm_labNA <- subset(dat_formatted_ref, is.na(label)) # %>% select(-sim) %>% left_join(rbh_sims)
mncm_labNA$set = "mncm_NA"


none_labNA %>%
  full_join(mncm_labNA, by = c("protein", "fasta", "seq1", "seq2", "aa1", "aa2"))
...
alldat_0 %>%
  ...
  mutate(diff = `No MNCM` - MNCM) %>%
  arrange(desc(diff))

figure <- plot_grid(a, b, ncol = 1, labels = c("A", "B"), align = "v", label_size = 8)
figure

 figure %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures//hist.png", ., height=3, width = 3.5, units = "in", dpi = 600)
 figure %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures//hist.pdf", ., height=3, width = 3.5, units = "in")
```
...
f1_supp <- prec_rec_sims%>% filter(set %in% c("MNCM", "BC")) %>% select(protein, f1, set, same, seqsim, sim) %>%
  ...
  geom_point(alpha=0.3) + xlab("Cosine similarity") + ylab("Batch corrected - uncorrected \n (F1 score)") +
...
f1_supp %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/f1_supp.png", ., height=1.5, width = 2, units = "in", dpi = 600)
f1_supp %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/f1_supp.pdf", ., height=1.5, width = 2, units = "in")
