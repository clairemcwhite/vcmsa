...
order3 <- c("extra vs. vcMSA", "MAFFT-FFTNS1 vs. vcMSA" ,  'FAMSA vs. vcMSA', 'MUSCLE vs. vcMSA', "CLUSTALO vs. vcMSA", "MAFFT-LINSI vs. vcMSA", "UPP vs. vcMSA", "TCOFFEE vs. vcMSA")



plot_16_main <- score_compare_aln_evaluate_20_nopca_16layer_t5_nopad %>% filter( alg %in% c("MAFFT-FFTNS1" ,  'FAMSA', "CLUSTALO", "MUSCLE", "MAFFT-LINSI", "UPP", "TCOFFEE")) %>%
                                                                   bind_rows(add_extra) %>%
...
order4 <- c("extra vs. vcMSA",'PROBCONS vs. vcMSA',"MAFFT-GINSI vs. vcMSA")

plot_16_supp <- score_compare_aln_evaluate_20_nopca_16layer_t5_nopad %>% filter( alg %in% c("PROBCONS", "MAFFT-GINSI")) %>%
                                                                   bind_rows(add_extra) %>%
  perform_plot_fxn(order4)
plot_16_supp %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/plot_16_supp.png", ., height = 2.3, width = 5.8,, units = "in", dpi = 600)
plot_16_supp%>%  ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/plot_16_supp.pdf", ., height = 2.3, width = 5.8, units = "in")
...
padding_plot <- score_compare_aln_evaluate_20_nopca_16layer_t5_padding0_nobc_mis %>% 
  left_join(score_compare_aln_evaluate_20_nopca_16layer_t5_padding10_nobc_mis, by= c("prot" ,"score_type")) %>%
  ggplot(aes(x = vcMSA.x, y = vcMSA.y, label = prot)) +
  geom_abline() + 
  geom_point(size= 1)  +
  xlab("No padding") + 
  ylab("Padding (10X's)")

padding_plot %>% ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/padding_plot.png", ., height = 2.5, width = 2.5, units = "in", dpi = 600)
padding_plot%>%  ggsave("/scratch/gpfs/cmcwhite/aln_comparison/figures/padding_plot.pdf", ., height = 2.5, width = 2.5, units = "in")
